...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const saveItem = async () => {
        ...
        try {
            if (_item.id) {
                await EtatRendezVousService.update(_item).then((response) => {
                    console.log(response.data);
                    update(response.data);
                    onClose();
                });
                MessageService.showToast(showToast, { severity: 'success', summary: 'Successful', detail: 'EtatRendezVous Updated', life: 3000 });
            }
        } catch (error) {
            console.log(error);
            MessageService.showToast(showToast, { severity: 'Error', summary: 'Error', detail: 'Failed to save etatRendezVous', life: 3000 });
        }
    };
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="EtatRendezVous" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog}>
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="EtatRendezVous">
