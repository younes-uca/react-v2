...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const saveItem = async () => {
        ...
        try {
            if (_item.id) {
                await CategorieRdvService.update(_item).then((response) => {
                    console.log(response.data);
                    update(response.data);
                    onClose();
                });
                MessageService.showToast(showToast, { severity: 'success', summary: 'Successful', detail: 'CategorieRdv Updated', life: 3000 });
            }
        } catch (error) {
            console.log(error);
            MessageService.showToast(showToast, { severity: 'Error', summary: 'Error', detail: 'Failed to save categorieRdv', life: 3000 });
        }
    };
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="CategorieRdv" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog}>
            ...
            <TabPanel header="CategorieRdv">
                <div className="formgrid grid">
                    ...
                    <div className="field col-6">
                        ...
                        <InputNumber id="nombreDose" value={item ? item.nombreDose : 0} onChange={(e) => onInputNumerChange(e, 'nombreDose')}/>
                    </div>
                    <div className="field col-6">
                        <label htmlFor="dureeDose">DureeDose</label>
                        <InputNumber id="dureeDose" value={item ? item.dureeDose : 0} onChange={(e) => onInputNumerChange(e, 'dureeDose')}/>
                    </div>
                </div>
            < /TabPanel>
        </TabView>
