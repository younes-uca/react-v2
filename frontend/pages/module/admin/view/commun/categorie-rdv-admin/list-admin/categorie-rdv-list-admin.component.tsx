...
const List = () => {
    ...
    const [items, setItems] = useState<CategorieRdvDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<CategorieRdvDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<CategorieRdvDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<CategorieRdvDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new CategorieRdvCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<CategorieRdvDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new CategorieRdvCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await CategorieRdvService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: CategorieRdvDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: CategorieRdvDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: CategorieRdvDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: CategorieRdvDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await CategorieRdvService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'CategorieRdv Deleted', life: 3000 });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await CategorieRdvService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'CategorieRdvs Deleted', life: 3000 });
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: CategorieRdvDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage categorie rdvs</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <Card>
                    <div className="search-container">
                        <div className="grid">
                                        ...
                                        <span className="p-float-label mr-3 align-search-items mt-4">
                                        <InputNumber id="3-1" value={criteria.nombreDoseMin} onChange={(e) => setCriteria({ ...criteria, nombreDoseMin: e.value })} mode="decimal" />
                                        <label htmlFor="3-1">NombreDose Min</label>
                                        </span>
                                        <span className="p-float-label mr-3 align-search-items mt-4">
                                        <InputNumber id="3-2" value={criteria.nombreDoseMax} onChange={(e) => setCriteria({ ...criteria, nombreDoseMax: e.value })} mode="decimal" />
                                        <label htmlFor="3-2">NombreDose Max</label>
                                        </span>
                                        <span className="p-float-label mr-3 align-search-items mt-4">
                                        <InputNumber id="4-1" value={criteria.dureeDoseMin} onChange={(e) => setCriteria({ ...criteria, dureeDoseMin: e.value })} mode="decimal" />
                                        <label htmlFor="4-1">DureeDose Min</label>
                                        </span>
                                        <span className="p-float-label mr-3 align-search-items mt-4">
                                        <InputNumber id="4-2" value={criteria.dureeDoseMax} onChange={(e) => setCriteria({ ...criteria, dureeDoseMax: e.value })} mode="decimal" />
                                        <label htmlFor="4-2">DureeDose Max</label>
                                        </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as CategorieRdvDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    <Column selectionMode="multiple" headerStyle={{ width: '4rem' }}> </Column>
                    
                    <Column field="code" header="Code" sortable headerStyle={{ minWidth: '15rem' }}></Column>
                    
                    
                    <Column field="libelle" header="Libelle" sortable headerStyle={{ minWidth: '15rem' }}></Column>
                    
                    
                    <Column field="nombreDose" header="NombreDose" sortable headerStyle={{ minWidth: '15rem' }}></Column>
                    
                    
                    <Column field="dureeDose" header="DureeDose" sortable headerStyle={{ minWidth: '15rem' }}></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete categorie rdv <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected categorie rdvs?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
