...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const [items, setItems] = useState<CategorieRdvDto[]>([list]);
    ...
    const saveItem = async () => {
        ...
        let _item = {...item};
        if (!_item.id) {
            const response = await CategorieRdvService.save(_item);
			_item.id = response.data.id;
            _items.push(_item);
            add(_item);
            MessageService.showToast(showToast, { severity: 'success', summary: 'Successful', detail: 'CategorieRdv Created', life: 3000 });
        }
        setItems(_items);
        onClose();
        setItem(emptyItem);
        ...
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="CategorieRdv">
                <div className="formgrid grid">
                    ...
                    <InputNumber id="nombreDose" value={item.nombreDose} onChange={(e) => onInputNumerChange(e, 'nombreDose')} />
                    </div>
                    <div className="field col-6">
                    <label htmlFor="dureeDose">DureeDose</label>
                    <InputNumber id="dureeDose" value={item.dureeDose} onChange={(e) => onInputNumerChange(e, 'dureeDose')} />
                    </div>
                </div>
            </TabPanel>
        </TabView>
