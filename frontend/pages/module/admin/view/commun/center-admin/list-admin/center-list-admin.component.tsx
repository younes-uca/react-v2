...
import Edit from '/pages/module/admin/view/commun/center-admin/edit-admin/center-edit-admin.component';
import Create from '/pages/module/admin/view/commun/center-admin/create-admin/center-create-admin.component';
import View from '/pages/module/admin/view/commun/center-admin/view-admin/center-view-admin.component';

const List = () => {

    const emptyItem = new CenterDto();
    const [items, setItems] = useState<CenterDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<CenterDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<CenterDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<CenterDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new CenterCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<CenterDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new CenterCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await CenterService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: CenterDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: CenterDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: CenterDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: CenterDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await CenterService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'Center Deleted', life: 3000 });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await CenterService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'Centers Deleted', life: 3000 });
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: CenterDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage centers</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <Card>
                    <div className="search-container">
                        <div className="grid">
                                        ...
                                        <span className="p-float-label mr-3 align-search-items mt-4">
                                        <Dropdown id="3" value={criteria.quartier} options={quartiers} onChange={(e) => setCriteria({ ...criteria, quartier: e.target.value })} optionLabel="libelle" filter showClear placeholder="Quartier" />
                                        </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as CenterDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="quartier.libelle" header="Quartier" sortable headerStyle={{ minWidth: '15rem' }}></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete center <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected centers?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
